...
上传前必须选定 **招聘岗位**。每个岗位绑定一个检验标准（招聘 Skill）。当前有 `intern` / 实习生、`image_software_test` / 影像 && 软件 测试，定义在 `backend/internal/skills/skills.go`；在招岗位在 `positions` 表，侧栏「岗位阶梯」可增改删。新增检验标准时在 skills.go 登记并补对应评分 prompt。
